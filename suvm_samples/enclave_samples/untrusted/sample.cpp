...
volatile int thread_start=0, thread_finish=0, ecall_thread_return;
void* pool_ptr_test;

void* test_services_thread(void* arg)
{
	int res;
	thread_start++;

	while (thread_start < 4) {}

	sgx_status_t ret = ecall_samples_sample(global_eid, &res, pool_ptr_test);

	ecall_thread_return += res;
	thread_finish++;
}
...
int SGX_CDECL main(int argc, char *argv[])
{
    ...
    pool_ptr_test = get_pool_ptr_debug();

    ret = ecall_samples_sample(global_eid, &ecall_return, pool_ptr_test);
    ...
    // Now run tests with multi-threading
    pthread_t testing_threads[4];
    for (int i=0;i<4;i++)
    {
		int res = pthread_create(&testing_threads[i], NULL, test_services_thread, NULL);
    ...
    if (ecall_return == 0 && ecall_thread_return == 0)
	{
