...
#define IMAGE_FILES_PATH L"./images_raw/"
...
int ocall_readfile(char *__buf, size_t __nbytes)
{
	std::ifstream file(ffd.cFileName, std::ios::binary | std::ios::ate);
	std::streamsize size = file.tellg();
	file.seekg(0, std::ios::beg);

	if (size != __nbytes || !file.read(__buf, size))
	{
		printf("Error: Unexpected file length found\n");
		exit(-1);
	}
...
int ocall_FindFirstFile()
{
	...
	StringCchCopy(szDir, MAX_PATH, IMAGE_FILES_PATH);
	StringCchCat(szDir, MAX_PATH, TEXT("\\*"));
	hFind = FindFirstFile(szDir, &ffd);

	if (INVALID_HANDLE_VALUE == hFind || ((ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0))
	{
		printf("Invalid dir recieved: %ls\n", IMAGE_FILES_PATH);
		return -1;
	}
...
{
	int retval = 0;
	ecall_open_server(global_eid, &retval);
}
...
int SGX_CDECL main(int argc, char *argv[])
{
	...
	*/
	...
	ecall_status = ecall_train_server(global_eid, &enclave_retVal, g_request_queue, g_response_queue);
	assert(ecall_status == SGX_SUCCESS && enclave_retVal == 0);
	
	// start the server on a background thread
	std::thread t(open_server);
	
	// main thread will handle connections
	runServer(do_trusted_logic, send_answer);
