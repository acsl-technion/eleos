...
static sgx_status_t SGX_CDECL sgx_ecall_test_suvm(void* pms)
{
	ms_ecall_test_suvm_t* ms = SGX_CAST(ms_ecall_test_suvm_t*, pms);
	sgx_status_t status = SGX_SUCCESS;

	CHECK_REF_POINTER(pms, sizeof(ms_ecall_test_suvm_t));

	ms->ms_retval = ecall_test_suvm();


	return status;
}

static sgx_status_t SGX_CDECL sgx_ecall_test_sgx(void* pms)
{
	ms_ecall_test_sgx_t* ms = SGX_CAST(ms_ecall_test_sgx_t*, pms);
	sgx_status_t status = SGX_SUCCESS;

	CHECK_REF_POINTER(pms, sizeof(ms_ecall_test_sgx_t));

	ms->ms_retval = ecall_test_sgx();
...
SGX_EXTERNC const struct {
	...
	3,
	{
		{(void*)(uintptr_t)sgx_ecall_test_suvm, 0},
		{(void*)(uintptr_t)sgx_ecall_test_sgx, 0},
